...
function ($, window, document, undefined) {
    function Plugin(element, options) {
        ...
        }).appendTo(toggleContainer), this.init()
    }
    ...
    $.extend(Plugin.prototype, {
        init: function () {
            var that = this;
            this.render(), $(window).smartresize(function () {
                that.render()
            })
        },
        render: function () {
        ...
        doGrid: function () {
            ...
            $.each(this.elems, function (current) {
                if (current > 0) {
                    ...
                    newLeft + this.width > width ? (left = 0, top += that.height + that.margin) : left = newLeft
                }
                that.animate(this, top, left, this.width, that.height, delay), that.initialized === !0 && (delay += that.delay)
            }), window.setTimeout(function () {
